...
def exchangeable_value(budget: float, exchange_rate: float, spread: int, denomination: int) -> int:
    ...
    total_after_exchange = int(exchange_money(budget, total_exchange_rate))

    return int(total_after_exchange - get_leftover_of_bills(total_after_exchange, denomination))
